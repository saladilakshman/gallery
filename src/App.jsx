...
function App() {
  ...
  const datafetching = async (query) => {
    axios
      .get(
        `https://pixabay.com/api/?key=${API_KEY}&category=${query}&image_type=photo`
      )
      .then((res) => {
      ...
      <Container sx={{ marginBlockEnd: 3 }}>
        ...
        >
          <Typography
            ...
            color="#357266"
          >
            Find your favourite Images here!
          </Typography>
          {alt ? (
            <Skeleton
              variant="rectangular"
        ...
        <Autocomplete
          ...
          renderInput={(params) => (
            <TextField label="select category" {...params} />
          )}
        ...
        <Dialog open={close} onClose={closedialog} sx={{ bgcolor: "#342E37" }}>
          <AppBar sx={{ bgcolor: "#342E37", boxShadow: "none" }}>
            <Stack
              ...
              alignItems="center"
              gap={isMobile ? 0 : 2}
              sx={{ marginInlineEnd: isMobile ? 0 : 4 }}
            >
              <IconButton
                color="secondary"
                onClick={async () => {
                  ...
                  await fetch(eventsrc)
                    .then((res) =>
                      res.blob().then((data) => {
                        ...
                        anchor.href = url;
                        anchor.download = `image${Math.random().toFixed(
                          2
                        )}.jpeg`;
                  ...
                  await fetch(eventsrc)
                    .then((res) =>
                      res
                        .blob()
                        .then((myblob) => {
                          const file = new File([myblob], "share.jpeg", {
                            type: myblob.type,
                          });
                          const data = {
                            files: [file],
